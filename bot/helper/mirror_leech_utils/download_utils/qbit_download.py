...
from bot import download_dict, download_dict_lock, xnox_client, LOGGER, config_dict, non_queued_dl, queue_dict_lock
...
async def add_qb_torrent(link, path, listener, ratio, seed_time):
    ...
    try:
        ...
        op = await sync_to_async(xnox_client.torrents_add, url, tpath, path, is_paused=added_to_queue, tags=f'{listener.uid}',
                                 ratio_limit=ratio, seeding_time_limit=seed_time, headers={'user-agent': 'Wget/1.12'})
        if op.lower() == "ok.":
            tor_info = await sync_to_async(xnox_client.torrents_info, tag=f'{listener.uid}')
            if len(tor_info) == 0:
                while True:
                    tor_info = await sync_to_async(xnox_client.torrents_info, tag=f'{listener.uid}')
                    if len(tor_info) > 0:
