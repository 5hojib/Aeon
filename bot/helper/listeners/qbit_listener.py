...
from bot import (LOGGER, QbInterval, QbTorrents, config_dict, download_dict,
                 download_dict_lock, get_client, qb_listener_lock)
...
async def __size_checked(tor):
    ...
    if hasattr(download, 'listener'):
        ...
        if limit_exceeded:
            ...
            await __onDownloadError(fmsg, tor)
...
async def onDownloadStart(tag):
    async with qb_listener_lock:
        QbTorrents[tag] = {'stalled_time': time(), 'stop_dup_check': False, 'size_checked': False, 'rechecked': False, 'uploaded': False, 'seeding': False}
        if not QbInterval:
