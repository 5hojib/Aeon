...
async def rclone_serve_booter():
    if not config_dict['RCLONE_SERVE_URL'] or not await aiopath.exists('rcl.conf'):
        if RcloneServe:
            try:
                RcloneServe[0].kill()
                RcloneServe.clear()
        ...
        return
    config = ConfigParser()
    async with aiopen('rcl.conf', 'r') as f:
    ...
    if RcloneServe:
        try:
            ...
            pass
    cmd = ["rclone", "serve", "http", "--config", "rcl.conf", "--no-modtime", "combine:", "--addr", f":{config_dict['RCLONE_SERVE_PORT']}", "--vfs-cache-mode", "full", "--vfs-cache-max-age", "1m0s", "--buffer-size", "64M"]
